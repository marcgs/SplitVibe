...
describe("PATCH /api/expenses/[id]", () => {
  ...
  it("updates a $90 expense to $60 with new even splits", async () => {
    ...
    const updateCall = txMock.expense.update.mock.calls[0][0];
    ...
    expect(updateCall.data.payers.create).toMatchObject({
      userId: "user-1",
      amount: 60,
    });
  });
  ...
  it("returns 403 when caller is not the creator", async () => {
    ...
    expect(txMock.expense.update).not.toHaveBeenCalled();
  });
...
describe("DELETE /api/expenses/[id]", () => {
  ...
  it("soft-deletes the expense for the creator (sets deletedAt)", async () => {
